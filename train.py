...
def ae_loss(model, x):
    ...
    loss = criterion(pred,x)/x.shape[0]
...
def vae_loss(model, x, beta = 1):
    ...
    # latent = torch.normal(mean=mu , std= torch.exp(log_std))
    latent = torch.randn_like(mu)*torch.exp(log_std)+ mu
    pred = model.decoder(latent)
    # print(x.shape)
    # print(pred.shape)
    log_var = 2*log_std
    recon_loss = criterion(pred,x)/len(x)
    
    # print('shape of criterion = ',recon_loss.shape)
    # input()
    kl_loss = 0.5 * torch.sum(torch.pow(mu,2) + torch.exp(2 * log_std) - 2 * log_std  - 1)/x.shape[0]
 
    # print('kl-loss mean', kl_loss_mean.shape)
    # input()
